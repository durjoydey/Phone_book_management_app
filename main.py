    ...
    if choice == 1:
        ...
        for x in range(i):
            if name[x] == search_name:
                print(search_name, "'s contact number is", phone[x], ",address is", address[x], "and email is",
                      email[x])
                flag = 1
        if flag == 0:
            print("Name wasn't found.\nPlease add this new entry.")
            name.append(str(input("Enter Name: ")))
            phone.append(int(input("Enter Number: ")))
            address.append(str(input("Enter Address: ")))
            email.append(str(input("Enter Email: ")))
        ...
        phonebook = str(input("Enter the contact you want to delete:"))
        i = len(name)
        for x in range(i):
            if name[x] == phonebook:
                if x == i - 1:  # deleting last contact
                    z += 1
                else:
                    name[x] = name[x + 1]
                    phone[x] = phone[x + 1]
